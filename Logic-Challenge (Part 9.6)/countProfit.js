...
function countProfit(shoppers) {
  ...
  for (let i = 0; i < listBarang.length; i++) {
    ...
    let totalProfit = listBarang[i][1];
    shoppers.forEach((data) => {
      let profit = 0;
      if (
        data.product === objOfItem.product &&
        objOfItem.leftOver >= data.amount
      ) {
        objOfItem.shoppers.push(data.name);
        objOfItem.leftOver -= data.amount;
        profit = totalProfit * data.amount;
      }
      objOfItem.totalProfit += profit;
    });
    result.push(objOfItem);
  }
  return result;
}
