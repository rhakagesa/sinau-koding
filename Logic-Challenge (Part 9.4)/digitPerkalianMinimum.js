...
function digitPerkalianMinimum(angka) {
  ...
  for (let i = 1; i <= angka; i++) {
    const checker = Math.round(angka / i);
    if (angka % i === 0 && checker * i === angka) {
      prevNum.push([i, checker]);
    }
  }
  const newNumber = prevNum.map((number) => number.join(""));
  let result = newNumber[0];
  for (let i = 1; i < newNumber.length; i++) {
    if (newNumber[i].length < result.length) {
      result = newNumber[i];
    }
  }
