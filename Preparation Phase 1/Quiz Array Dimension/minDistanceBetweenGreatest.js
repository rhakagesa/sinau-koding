...
function minDistanceBetweenGreatest(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] === maxNum) result[0].push(i);
  }

  result[0].sort((a, b) => b - a);

  for (let j = 0; j < result[0].length; j++) {
    const subtarct = result[0][j] - result[0][j + 1];
    if (!Number.isNaN(subtarct)) result[1].push(subtarct);
  }

  return result[1].length > 1 ? Math.min(...result[1]) : 0;
}
